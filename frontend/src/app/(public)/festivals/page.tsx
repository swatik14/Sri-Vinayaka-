        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex gap-3 mb-8">
            <button
              ...
              className={`px-5 py-2.5 rounded-full font-semibold text-sm transition-all ${activeTab === 'upcoming' ? 'bg-temple-maroon text-white' : 'bg-temple-cream-dark text-temple-brown hover:bg-temple-cream'}`}
            >
              {t('upcomingEvents')} ({upcoming.length})
            </button>
            <button
              onClick={() => setActiveTab('all')}
              className={`px-5 py-2.5 rounded-full font-semibold text-sm transition-all ${activeTab === 'all' ? 'bg-temple-maroon text-white' : 'bg-temple-cream-dark text-temple-brown hover:bg-temple-cream'}`}
            >
            ...
            <div className="space-y-5">
              {display.map((f) => {
                ...
                return (
                  <div
                    key={f.id}
                    className={`card overflow-hidden flex flex-col md:flex-row ${f.is_featured ? 'ring-2 ring-temple-gold' : ''}`}
                  >
                    {/* Date column */}
                    <div className={`${isUpcoming ? 'bg-temple-maroon' : 'bg-gray-400'} text-white p-6 flex flex-col items-center justify-center min-w-[100px] md:min-w-[120px] text-center flex-shrink-0`}>
                      <p className="text-3xl font-heading font-bold text-temple-gold">{format(date, 'd')}</p>
                      ...
                      {f.is_featured && isUpcoming && (
                        <span className="mt-2 text-[10px] font-bold bg-temple-gold text-temple-brown px-2 py-0.5 rounded-full">
                          ★ FEATURED
                        </span>
                      )}
                      {!isUpcoming && (
